...
function config_image_hook__orangepi-5-max() {
    ...
    if [ "${suite}" == "jammy" ] || [ "${suite}" == "noble" ] || [ "${suite}" == "oracular" ] || [ "${suite}" == "plucky" ]; then
        ...
        # ===================== 修复后的BCMDHD DKMS安装逻辑 =====================
        # Install BCMDHD PCIe/SDIO/USB DKMS packages (修复通配符安装问题)
        # 定义deb包URL列表
        ...
        for deb_url in "${BCMDHD_DEB_URLS[@]}"; do
            ...
            # 复制deb包到rootfs临时目录
            cp "${deb_host_path}" "${rootfs}${deb_rootfs_path}" || {
                echo "Error: 无法复制deb包到rootfs: ${deb_filename}" >&2
                rm -rf "${HOST_TMP_DIR}" "${rootfs}${ROOTFS_TMP_DIR}"
                return 1
            }
            
            # 记录rootfs内的包路径（用于后续安装）
            deb_paths+=("${deb_rootfs_path}")
        done
        
        # 验证rootfs内的deb包是否存在
        for deb_path in "${deb_paths[@]}"; do
            if [ ! -f "${rootfs}${deb_path}" ]; then
                echo "Error: rootfs内未找到deb包: ${deb_path}" >&2
                rm -rf "${HOST_TMP_DIR}" "${rootfs}${ROOTFS_TMP_DIR}"
                return 1
            fi
        done
        
        # 在chroot环境内安装DKMS依赖和deb包（修复核心：不用通配符，逐个指定包）
        echo "Installing BCMDHD DKMS packages in rootfs..."
        # 先安装dkms依赖
        chroot "${rootfs}" apt-get update -q || echo "Warning: apt update 警告（不影响安装）"
        chroot "${rootfs}" apt-get install -y --no-install-recommends dkms || {
            echo "Error: 安装DKMS依赖失败" >&2
            rm -rf "${HOST_TMP_DIR}" "${rootfs}${ROOTFS_TMP_DIR}"
            return 1
        }
        
        # 方式1：用dpkg -i安装本地包（推荐，避免apt通配符问题），再修复依赖
        chroot "${rootfs}" dpkg -i "${deb_paths[@]}" || {
            echo "Warning: dpkg安装临时失败，尝试修复依赖..." >&2
            chroot "${rootfs}" apt-get -f install -y || {
                echo "Error: 修复DKMS包依赖失败" >&2
                rm -rf "${HOST_TMP_DIR}" "${rootfs}${ROOTFS_TMP_DIR}"
                return 1
            }
        }
        ...
        # 验证安装结果
        if ! chroot "${rootfs}" dpkg -l | grep -q bcmdhd-dkms; then
            echo "Error: BCMDHD DKMS包安装后未检测到" >&2
            rm -rf "${HOST_TMP_DIR}" "${rootfs}${ROOTFS_TMP_DIR}"
            return 1
        fi
